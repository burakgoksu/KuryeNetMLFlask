...
class KuryeNetML:
    ...
    def __init__(self,csvFile):
        ...
        self.X = df[
            ['Delivery_Person_Age', 'Weather_Condition', 'Road_Traffic_Density', 'Type_Of_Order', 'Type_Of_Vehicle',
             'Multiple_Deliveries', 'City', 'Distance', 'Day_Type', 'Time_Category']]
        self.y = df['Time_Taken']

    def TrainCatBoostModel(self):
        """
        # Veri setini yükle
        df = pd.read_csv(csvFile)

        # Bağımsız değişkenler (X) ve bağımlı değişken (y) seçimi
        self.X = df[['Delivery_Person_Age', 'Weather_Condition', 'Road_Traffic_Density', 'Type_Of_Order', 'Type_Of_Vehicle',
                'Multiple_Deliveries', 'City', 'Distance', 'Day_Type', 'Time_Category']]
        self.y = df['Time_Taken']
        ...
        cb_model = CatBoostRegressor(iterations=1000, learning_rate=0.01, depth=8, loss_function='RMSE', verbose=100,
                                     l2_leaf_reg=1)
    ...
    def Prediction(self,Delivery_Person_Age,Weather_Condition,Road_Traffic_Density,Type_Of_Order,Type_Of_Vehicle,Multiple_Deliveries,City,Distance,Day_Type,Time_Category):
        ...
        X_encoded_columns = pd.get_dummies(self.X).columns
        ...
        print(f"Tahmini Süre: {prediction_value} dakika")
